...
def writeToFile():
    print("Fájlba írás")
    file=open("bpBonus.txt","w",encoding="utf-8")
    sumBonus=0
    for worker in workerList:
        if(len(worker)!=1 and worker[1]=="Budapest"):
            sumBonus+=int(worker[4])
    file.write("A budapestiek jutalma: ")    
    file.write(str(sumBonus))
    file.close()
    print("Írás sikeres")
readFile()
countWorker()
